    ...
    const loadStats = async () => {
      try {
        ...
        const statsWithIcons = data.map(item => {
          if (item.title === 'Total Revenue') return { ...item, icon: DollarSign };
        ...
        <main className="flex-1 overflow-y-auto p-8 z-0">
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {loading ? (
                [1, 2, 3, 4].map(i => (
                  <div key={i} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6 h-32 animate-pulse flex items-center justify-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl h-96 p-6 backdrop-blur-sm">
                ...
                <div className="space-y-4">
                  {[1, 2, 3, 4, 5].map(i => (
                    <div key={i} className="flex items-center space-x-4 p-3 hover:bg-slate-700/30 rounded-xl transition-colors cursor-pointer group">
